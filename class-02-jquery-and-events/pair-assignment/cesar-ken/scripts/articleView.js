...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    if ($(this).val()) {
      // DONE - CRJ: If the select box was changed to an option that has a value, we need to hide all the articles,
      ...
      // DONE - CRJ: If the select box was changed to an option that is blank, we should
...
articleView.handleCategoryFilter = function() {
  // DONE - CRJ: Just like we do for #author-filter above, we should handle change events on the #category-filter element.
...
articleView.handleMainNav = function() {
  // DONE - CRJ: Add an event handler to .main-nav element that will power the Tabs feature.
...
articleView.setTeasers = function() {
  ...
  // DONE - CRJ: Add an event handler to reveal all the hidden elements,
  //       when the .read-on link is clicked. You can go ahead and hide the
  //       "Read On" link once it has been clicked. Be sure to prevent the default link-click action!
  //       Ideally, we'd attach this as just 1 event handler on the #articles section, and let it
  //       process any .read-on clicks that happen within child nodes.

  $('section#articles').on('click.read-on', function(e) {
    e.preventDefault();
    var readOnLink = $(e.target);

    readOnLink.hide();
    readOnLink.parent().find('*:nth-of-type(n+2)').show();
  });


};

// DONE - CRJ: Call all of the above functions, once we are sure the DOM is ready.
